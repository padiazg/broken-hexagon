...
// SplashCheckFn is a function that verifies output from a test case.
...
func TestSplash(t *testing.T) {
	...
	checkContains := func(want string) SplashCheckFn {
		return func(t *testing.T, std, _ string) {
			...
			if !strings.Contains(std, want) {
				t.Errorf("expected %q in stdout, got: %q", want, std)
			}
		}
	}
	...
	checkNotContains := func(want string) SplashCheckFn {
		return func(t *testing.T, std, _ string) {
			t.Helper()
			if strings.Contains(std, want) {
				t.Errorf("expected %q NOT to appear in stdout, but it was: %q", want, std)
			}
		}
	}

	checkError := func(want string) SplashCheckFn {
		return func(t *testing.T, _, errStr string) {
			t.Helper()
			if want != "" {
				if !strings.Contains(errStr, want) {
					t.Errorf("expected %q in error output, got: %q", want, errStr)
				}
				return
			}
			if errStr != "" {
				t.Errorf("unexpected error output: %q", errStr)
			}
		}
	}
	...
	tests := []struct {
		...
		},
		{
			...
			buff: &errWriter{err: io.ErrClosedPipe},
			version: &VersionInfo{
				Version:   "v0.0.1",
				Major:     0,
				Minor:     0,
